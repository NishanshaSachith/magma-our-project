...
          try {
            await api.put(`/jobhome-technicians/${selectedJob.id}/state`, { state: 'opened' });
...
      try {
        const response = await api.get("/jobhome-technicians/states");
...
        console.log("Fetched items (services):", response.data);
...
      // Ensure the dropdown closes if the click is outside the button/menu area
      const dropdown = document.querySelector('.create-job-dropdown-container');
...
              />
            </div>
            {/* BEGIN: Create Job Card Dropdown */}
...
                      {/* Conditional rendering for services/items */}
                      {items.length === 0 ? (
...
                  {/* IMPROVEMENT: Always show Created on, and optionally show Cancelled on */}
                  <div className={`flex flex-col items-end leading-snug ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    <div>Created on: {formatDateTime(job.createdAt)}</div>
                    {job.status === "Cancelled" && job.cancellationDateTime && (
                      <div className={`text-xs italic ${isDarkMode ? 'text-red-400' : 'text-red-600'}`}>
                        Cancelled: {formatDateTime(job.cancellationDateTime)}
                      </div>
                    )}
                  </div>
                  {CurrentStatusIcon && (
